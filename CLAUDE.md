...
- **A site-wide `htmx:afterSettle` handler must check WHAT was swapped.** `afterSettle` on `document.body` fires for *every* swap on the page — each feed lane, each SSE frame, each deferred panel, the sidebar's own account fetch — not just navigations. site.js closed the mobile sidebar on every one of them, so opening the sidebar while anything was in flight slammed it shut the moment that request landed; the character page's six panels and the superiors page's two halves each did it. A navigation is identifiable: every sidebar link carries `hx-target="#hx-page-container"`, so the handler gates on `evt.detail.target.id === 'hx-page-container'` — the same check `roll-ticker.js` uses to re-init after an in-app navigation.
