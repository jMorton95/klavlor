...
The one place a card's TOTAL is consulted is `ILootFeedService.ExceedsTier`, and it is not an exception to that rule: it decides *presentation only*. A card whose total outgrew its lane's ceiling — three 90M drops from one source merge into one Epic card worth 270M — is marked in **its own** tier colour (`.tier-overflow` plus a `.tier-overflow-{tier}` hue variable in `app.css`, driven by `GetTierOverflowClasses`). The cue is **three layers, not a brighter border**: a tinted card body, a brighter outline (mixed toward white in dark mode, where the raw hue disappears against slate-950), and a detached pulsing ring standing clear of the card. The first version only brightened the border and pulsed a glow behind it, and in production dark mode it could not be found at all — every card in a swimlane already has a coloured border and ring in the same hue, so more of that colour in the same place reads as nothing. Any future change here has to differ from a neighbour in **shape or area**, not in brightness. The card is never promoted, so cheap drops still cannot fake a rarer lane; they can only make the lane they are in glow. Always false for Legendary, which has no ceiling. It yields to `.legendary-card` and `.giga-card`, which animate the same border — they can't collide in practice, since both a 250M+ drop and an injected special classify as Legendary, but the razor states the precedence rather than relying on stylesheet order. `KlavLor.UnitTests/LootFeedTierClassificationTests.cs` pins the boundaries and the "total never changes the lane" property.
