...
**Dry-board entry rules.** Not-yet-obtained items join the dry streak board as soon as the character has done enough rolls to have expected the drop once (`MinMissingMultiple` = 1.0) — a 1/100 item still missing at 101 rolls shows as 1× dry. Items already obtained need `MinMultiple` = 1.75, since a drop that arrived only fractionally late isn't worth a slot. The bottom-end rarity filter still drops anything more common than 1/100, or a 1× bar would flood the board with commons. `GetBoard` orders by tier descending then by `ExpectedKc` — not `RarityDenominator`, which a depth-modelled source stores as 0 and which therefore lost Doom every within-tier tie-break — so if the 200-row cap bites it trims the mildest streaks first.
