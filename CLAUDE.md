...
**A repeat drop's luck is measured from the PREVIOUS receipt, never from the absolute roll number.** `LootFeedDrop.RollsSincePrevious` carries that gap, resolved by `ILootRecordRepository.GetRollsSincePreviousReceipt` — one batched query shared by the live publish path (`LootIngestHandler`) and the backfill path (`LootFeedTiersHandler`), so the two cannot drift. It is null only on a first-ever receipt, where the absolute count already *is* the gap. Judging a repeat against its position reported a second 1/100 item at roll 200 as "2x dry" when the player had gone 150 rolls since the last one, and made every guaranteed drop absurd (an Atlatl dart from a 300th Lunar Chest read as 300x dry).

**Only items of 1/6 or rarer get a lucky/dry verdict** (`FeedLuckRules.WorthRating`). Below that, ordinary variance renders as a lurid multiple. The threshold reads **expected rolls** from `SourceLootService`, never the raw stored denominator — that is precisely what keeps raid uniques rated and is why Chambers of Xeric, Tombs of Amascut and Theatre of Blood need no special case: a CoX prayer scroll is stored as 20/69 (which as a bare fraction looks like 1 in 3.45 and would be filtered out), but it is a share of the unique table and `RaidUniqueShareStrategy` has already scaled it to ~110 raids by the time the feed sees it. `KlavLor.UnitTests/FeedLuckRulesTests.cs` pins this; `KlavLor.IntegrationTests/RollsSincePreviousReceiptTests.cs` pins the gap query.
