...
The one place a card's TOTAL is consulted is `ILootFeedService.ExceedsTier`, and it is not an exception to that rule: it decides *presentation only*. A card whose total outgrew its lane's ceiling — three 90M drops from one source merge into one Epic card worth 270M — pulses its border in **its own** tier colour (`.tier-overflow` plus a `.tier-overflow-{tier}` hue variable in `app.css`, driven by `GetTierOverflowClasses`). The card is never promoted, so cheap drops still cannot fake a rarer lane; they can only make the lane they are in glow. Always false for Legendary, which has no ceiling. It yields to `.legendary-card` and `.giga-card`, which animate the same border — they can't collide in practice, since both a 250M+ drop and an injected special classify as Legendary, but the razor states the precedence rather than relying on stylesheet order. `KlavLor.UnitTests/LootFeedTierClassificationTests.cs` pins the boundaries and the "total never changes the lane" property.
